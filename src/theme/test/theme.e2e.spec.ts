...
describe('ThemeController (E2E)', () => {
    ...
    describe('GET /theme/search', () => {
        it('테마를 검색한다.', async () => {
            ...
            for (const theme of response.body) {
                ...
                expect(theme.storeName).toBeDefined();
                expect(theme.storeLocation).toBeDefined();
                expect(theme.themeName).toBeDefined();
                expect(theme.image).toBeDefined();
                expect(theme.plot).toBeDefined();
                expect(theme.genre).toBeDefined();
                expect(theme.time).toBeDefined();
                expect(theme.level).toBeDefined();
                expect(theme.price).toBeDefined();
                expect(theme.reviewCount).toBeDefined();
            }
        });
    });
    ...
    describe('GET /theme', () => {
        ...
        it('테마 목록을 지역 별로 조회한다.', async () => {
            ...
            const theme = await themeService.getThemeById(1);
            const location = theme?.store.location;
    ...
    describe('GET /theme/{themeId}', () => {
        it('특정 테마 정보를 조회한다.', async () => {
            ...
            if (Array.isArray(response.body.reviews) && response.body.reviews.length !== 0) {
