...
describe('ReviewRepository', () => {
  ...
  const createTestReview = async (user: User, record: Record) => {
  ...
  beforeEach(async () => {
    ...
    theme = await themeRepository.save(
      new Theme({
    ...
    record = await recordRepository.save(
      await Record.create({
        user,
        theme,
        isSuccess: true,
    ...
    await recordRepository.saveTag(
      await Tag.create({
        user,
        record,
        isWriter: true,
  ...
  describe('save()', () => {
    it('리뷰 엔티티를 저장한다.', async () => {
      const review = await createTestReview(user, record);
  ...
  describe('findOne()', () => {
    ...
    it('userId와 recordId로 리뷰를 찾는다.', async () => {
      ...
      const result = await reviewRepository.findOneByUserIdAndRecordId(
        userId,
        recordId,
      );
  ...
  describe('countReviewsIn()', () => {
    it('기록의 리뷰 개수를 반환한다.', async () => {
      await createTestReview(user, record);
      const result = await reviewRepository.countReviewsInRecord(
        record.getId(),
      );
    ...
    it('테마의 공개 리뷰 개수를 반환한다.', async () => {
      await createTestReview(user, record);
      const result = await reviewRepository.countVisibleReviewsInTheme(
        theme.getId(),
      );
      expect(result).toBe(1);
    });
    ...
    it('매장의 공개 리뷰 개수를 반환한다.', async () => {
      await createTestReview(user, record);
      const result = await reviewRepository.countVisibleReviewsInStore(
        store.getId(),
      );
      expect(result).toBe(1);
    });
  ...
  describe('getVisibleReviewsInTheme()', () => {
    it('테마의 공개 리뷰 전체를 반환한다.', async () => {
      await createTestReview(user, record);
      const result = await reviewRepository.getVisibleReviewsInTheme(
        theme.getId(),
      );
    ...
    it('데이터가 존재하지 않으면 빈 배열을 반환한다.', async () => {
      const result = await reviewRepository.getVisibleReviewsInTheme(999);
